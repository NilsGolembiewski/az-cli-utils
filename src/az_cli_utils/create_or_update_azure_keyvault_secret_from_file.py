...
@click.option(
    "--secret-name", "-s", required=True, help="Name of the secret to create or update."
)
@click.option(
    "--file",
    "-f",
    ...
    help="Path to the file whose contents will be stored as the secret value.",
)
def main(vault_name, secret_name, file_path):
    ...
    try:
        ...
        click.echo(
            f"Secret '{secret_name}' set in vault '{vault_name}' from file '{file_path}'."
        )
    except Exception as e:
